...
%%
%% Tests macula_tweann_app, macula_tweann_sup, and tweann_logger modules.
%%
...
%% ============================================================================
%% macula_tweann_app Export Tests
%% ============================================================================

macula_tweann_app_exports_test() ->
    Exports = macula_tweann_app:module_info(exports),
    ?assert(lists:member({start, 2}, Exports)),
    ?assert(lists:member({stop, 1}, Exports)).

macula_tweann_app_behaviour_test() ->
    Behaviours = proplists:get_value(behaviour, macula_tweann_app:module_info(attributes), []),
    ?assert(lists:member(application, Behaviours)).

%% ============================================================================
%% macula_tweann_sup Export Tests
%% ============================================================================

macula_tweann_sup_exports_test() ->
    Exports = macula_tweann_sup:module_info(exports),
    ?assert(lists:member({start_link, 0}, Exports)),
    ?assert(lists:member({init, 1}, Exports)).

macula_tweann_sup_behaviour_test() ->
    Behaviours = proplists:get_value(behaviour, macula_tweann_sup:module_info(attributes), []),
...
application_can_start_test() ->
    ...
    ?assert(lists:keymember(macula_tweann, 1, Apps)).

application_info_test() ->
    %% Ensure application is loaded
    application:load(macula_tweann),
    %% Verify application metadata is accessible
    {ok, Description} = application:get_key(macula_tweann, description),
...
application_vsn_test() ->
    ...
    {ok, Vsn} = application:get_key(macula_tweann, vsn),
...
supervisor_children_test() ->
    ...
    Children = supervisor:which_children(macula_tweann_sup),
...
supervisor_restart_strategy_test() ->
    ...
    case macula_tweann_sup:init([]) of
